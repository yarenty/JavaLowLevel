 ...
 *      .html
 */
public final class SinglyLinkedListOrginal<E> extends AbstractSequentialList<E>
	 ...
	 * 
	 */
	public SinglyLinkedListOrginal() {
		first = new Node<E>(null, null);
		tail = new Node<E>(null, null);
		last = first;
		tail.next = last; // tail.next is always the node before the "end"
		first.next = last; // when list is empty first.next == last
		size = 0;
	}
	 ...
	 * @param elements
	 */
	public SinglyLinkedListOrginal(E... elements) {
		this();
		for (E e : elements)
			add(e);
	}
	 ...
	 */
	public SinglyLinkedListOrginal(Collection<? extends E> c) {
		this();
		addAll(c);
	}
	...
	private Node<E> addNode(Node<E> after, E value) {
		...
		after.next = n;
		if (n.next == last) // is "n" the new last node?
			tail.next = n; // tail is not a proper element of the list and must
							// be manually updated
		size++;
		modCount++;
		return n;
	...
	public boolean add(E e) {
		return addNode(tail.next, e) != null;
	}
	...
	public boolean addAll(int index, Collection<? extends E> c) {
		...
		return insertAllAfter(findNodeBefore(first, index), c);
	}
	 ...
	 */
	@Override
	public SinglyLinkedListOrginal<E> clone() {
		return new SinglyLinkedListOrginal<E>(this);
	}
	 ...
	 */
	public SinglyLinkedListOrginal<E> reverse() {
		if (size() > 1) {
			for (Node<E> current = first.next, previous = last; current != last;) {
				Node<E> next = current.next;
				current.next = previous;
				previous = current;
				current = next;
			}
			Node<E> begin = first.next;
	 ...
	 */
	public SinglyLinkedListOrginal<E> reverseRecursive() {
		if (size() > 1) {
			Node<E> oldBegin = first.next;
			recursion(first.next, first.next);
			first.next = tail.next;
			tail.next = oldBegin;
	...
	void checkInvariants(int expectedSize) {
		...
		final boolean empty = (first.next == last & isEmpty());
		t(size == 0 ? empty : !empty);
		
		t(tail.next.next == last);
